...
- 関西は暖色（神戸=赤茶 / 箕面=金 / トアロード=ばら）、関東は寒色
  （西新宿=緑 / 赤坂=空 / 麹町=青 / 千駄ヶ谷=紫）。色で地域が分かるようにしている
- `styles.css` の `--series-1`〜`-7`。**変えたら必ず再検証する**
  （dataviz スキルの `validate_palette.js`。現状は全項目パス）
- 店舗ではないもの（割合の棒・退会/休会の区分）は**無彩色**にする。
  8色すべて店舗に使っているため、色みを持たせるとどこかの店舗に見えてしまう
